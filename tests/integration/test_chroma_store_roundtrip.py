...
def test_settings(settings):
    """Use settings with test-specific configuration."""
    # Use the settings from config/settings.yaml
    # The persist_directory is already configured there
    return settings
...
def chroma_store(test_settings):
    ...
    # Cleanup: clear collection and close client after each test
    # This is important on Windows to release file locks
    try:
        store.clear()
    except Exception:
        # Collection may already be deleted or in error state
        pass
    
    # Close the ChromaDB client to release database connections
    try:
        if hasattr(store, 'client') and store.client:
            # ChromaDB PersistentClient may not have explicit close method
            # but we can try to access internal _client and close it
            if hasattr(store.client, '_client'):
                store.client._client.close()
    except Exception:
        # Closing may fail; this is not critical
        pass
